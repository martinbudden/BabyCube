...
_xMax = eX - 2*eSizeX - 55.8+15; // . For E3D hotend, was limited (at -52) by hotend fan hitting belts, now limited by E3D hotend fan hitting right motor mount
...
_zMin = _printBedSize == 100 ? 77.35 : _useCNC ? 71 : 59.5;
_zMax = eZ - (_printBedSize == 100 ? 61.4 : _useCNC ? 55 : 81);
...
// X-axis
function xPos(t) = t==-1 ? 0 : t<=3 ? (_xMin + _xMax)/2 : t==4 ? _xMin : t==5 ? _xMax : t==6 ? _xMax : t==7 ? _xMin : _xMax;
function xPosAnimate(t) = eX/2 + t*100;

// Y-axis
function yPos(t) = t==-1 ? 0 : t<=3 ? (_yMin + _yMax)/2 : t<=5 ? _yMin  : _yMax;
