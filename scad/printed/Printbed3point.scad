...
module printbed3pointFrame() {
    ...
    for (x = [_zRodSeparation/2 - eSize - scsSize.y/2 - 1 - bracketThickness, -_zRodSeparation/2 + scsSize.y/2 + 1 + bracketThickness])
        translate([x, -scsSize.x/2, -eSize/2])
            extrusionOY(yExtrusionLength, eSize);

    xExtrusionLength = (floor(heatedBedSize.x/50) + 1)*50;
    translate([-xExtrusionLength/2, yExtrusionLength - scsSize.x/2, -eSize/2])
...
module Printbed_Frame_stl() {
    ...
    stl("Printbed_Frame")
        color(pp1_colour)
        vflip() {
            translate([-endPieceSize.x/2, armSize.y - scsSize.x/2 + (eSize - endPieceSize.y)/2, -eSize/2]) {
                difference() {
            ...
            translate([-crossPieceSize.x/2, armSize.y/2, eSize/2])
                vflip()
                    zCarriageCrossPiece(crossPieceSize, armSize.x);

            difference() {
                translate([frameOffsetX - armSize.x, -scsSize.x/2, -eSize/2]) {
                    rounded_cube_xy(armSize + [0, 10, 0], fillet);
                    translate([armSize.x - sideBracketSize.x, 0, eSize - sideBracketSize.z])
                        rounded_cube_xy(sideBracketSize, fillet);
                    *translate([armSize.x - sideBracketSize.x, sideBracketSize.y, 0])
                        rotate([0, 90, 0])
            ...
            difference() {
                translate([-frameOffsetX, -scsSize.x/2, -eSize/2]) {
                    rounded_cube_xy(armSize + [0, 10, 0], fillet);
...
module printbed3point(leadnutOffset=eSize/2, scsOffset=0) {
    ...
    translate([_zRodSeparation/2, 0, scsOffset])
        rotate(-90)
    ...
    scsSize = scs_size(scs_type);
    yExtrusionLength = (floor(heatedBedSize.y/50) + 1)*50;
    translate([0, yExtrusionLength - heatedBedSize.y/2 + eSize/2 + heatedBedHoleOffset - scsSize.x/2, eSize/2]) {
        printBedSize = 120;
        corkUnderlay([heatedBedSize.x, heatedBedSize.y, 3], printBedSize)
            heatingPad([100, 100, 2.5])
                Heated_Bed()
